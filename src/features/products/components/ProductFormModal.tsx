...
export function ProductFormModal({ open, product, onClose }: ProductFormModalProps) {
  ...
  // ── Flat quantity (no color/size variants) ──────────────────────────────────
  const flatFileInputRef = useRef<HTMLInputElement>(null)
  const flatDragIndex = useRef<number | null>(null)
  const [flatStock, setFlatStock] = useState<number>(product?.stock ?? 0)
  const [flatImages, setFlatImages] = useState<VariantImage[]>(() =>
    (product?.images ?? []).map((url) => ({ preview: url, url }))
  )

  const MAX_FLAT_IMAGES = 5

  function handleFlatFiles(files: FileList | null) {
    if (!files || files.length === 0) return
    const slots = MAX_FLAT_IMAGES - flatImages.length
    if (slots <= 0) return
    ...
    setFlatImages((imgs) => [...imgs, ...added])
  }

  function removeFlatImage(i: number) {
    setFlatImages((imgs) => {
  ...
  function onFlatDragStart(i: number) {
    flatDragIndex.current = i
  }
  function onFlatDrop(i: number) {
    if (flatDragIndex.current === null || flatDragIndex.current === i) return
    setFlatImages((imgs) => {
      const next = [...imgs]
      const [moved] = next.splice(flatDragIndex.current as number, 1)
      next.splice(i, 0, moved)
      return next
    })
    flatDragIndex.current = null
  }
  ...
  const categoryHasVariants = selectedCategory?.hasVariants ?? true
  const categorySizeSet: SizeSetType = selectedCategory?.sizeSet ?? 'CLOTHING'
  const activeSizes = SIZE_SETS[categorySizeSet]
  ...
  async function syncVariants(productId: string, resolvedVariants: LocalVariant[]) {
    ...
    const sizesPayload = (v: LocalVariant) =>
      Object.entries(v.sizes).map(([size, stock]) => ({ size, stock }))
  ...
  async function onSubmit(data: ProductInput) {
    ...
    try {
      if (!categoryHasVariants) {
        // Sem variação: só quantidade + fotos do próprio produto
        if (flatImages.length === 0) {
          setSyncError('Adicione pelo menos uma foto do produto')
          return
        }
        const urls = await resolveVariantImages(flatImages)
        setFlatImages(urls.map((url) => ({ preview: url, url })))

        const payload: ProductInput = { ...data, stock: flatStock, images: urls }
        if (product) {
      ...
      <div className="bg-background relative flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-xl shadow-xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex min-h-0 flex-1 flex-col">
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="grid grid-cols-[1fr_1px_1fr]">
              ...
              <div className="space-y-4 p-6">
                ...
                <div className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      {categoryHasVariants ? (
                        ...
                        <>
                          <span
                            className={`text-2xl font-bold ${flatStock === 0 ? 'text-red-400' : flatStock <= 10 ? 'text-amber-500' : 'text-green-600'}`}
                          >
                            {flatStock}
                          </span>
                  ...
                  {categoryHasVariants && visibleVariants.length > 0 && (
            ...
            {/* ── Quantidade (sem variação de cor/tamanho) ────────────────── */}
            {!categoryHasVariants && (
              <div className="space-y-4 border-t px-6 py-5">
                <div className="flex items-center gap-2">
                  <Palette className="h-4 w-4 text-purple-500" />
                  <span className="text-sm font-semibold text-gray-700">Quantidade em estoque</span>
                </div>
                <p className="text-xs text-gray-400">
                  Esta categoria não usa cor/tamanho — informe apenas a quantidade disponível.
                </p>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    value={flatStock || ''}
                    placeholder="0"
                    onChange={(e) => setFlatStock(Math.max(0, parseInt(e.target.value) || 0))}
                    className="w-32 rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-700 focus:border-purple-400 focus:ring-1 focus:ring-purple-300/30 focus:outline-none"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {flatImages.length < MAX_FLAT_IMAGES ? (
                    <button
                      type="button"
                      ...
                      onDrop={(e) => {
                        e.preventDefault()
                        handleFlatFiles(e.dataTransfer.files)
                      }}
                    ...
                    >
                      <Upload className="h-4 w-4" /> Clique ou arraste · {flatImages.length}/
                      {MAX_FLAT_IMAGES} fotos
                    </button>
                  ) : (
                    <p className="text-center text-xs text-gray-400">
                      Limite de {MAX_FLAT_IMAGES} fotos atingido · Remova uma para adicionar outra
                    </p>
                  )}
                  <input
                    ...
                    multiple
                    className="hidden"
                    onChange={(e) => handleFlatFiles(e.target.files)}
                  />
                </div>
              </div>
            )}
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    {variants.map((v, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className="space-y-2 rounded-xl border border-gray-100 bg-gray-50/60 p-3"
                        >
                          ...
                          {/* Sizes + total (full width, wraps freely) */}
                          <div className="flex flex-wrap items-center gap-1 pl-10">
                            ...
                            {total > 0 && (
                              <p className="ml-1 text-xs font-bold text-gray-500">{total} peças</p>
                            )}
                            {total === 0 && (
                  ...
                  <div className="space-y-4 rounded-xl border border-purple-200 bg-purple-50/40 p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-[11px] text-gray-400">
                        Total desta cor:{' '}
                        {Object.values(variantForm.sizes).reduce((a, b) => a + b, 0)} peças
                      </p>
                    </div>
